...
namespace RegistroFacturasWEB.Seguridad
{
    public partial class Usuarios : System.Web.UI.Page
    {
        ...
        {
            CargarUsuariosGridBusqueda(txtUsuario.Text, txtNombreUsuario.Text, ddlDominio.SelectedValue.ToString());
        }
        ...
        private UsuarioDTO CargarObjetoUsuarios(ref UsuarioDTO _usuarioDto)
        {
            ...
            _usuarioDto.IDENTIFICADOR = ddlDominio.SelectedValue;
        ...
        private bool ValidaCampos()
        {
            ...
            if ((txtUsuario.Text != "") && (txtUsuario.Text.Length <= 30)) { result = true; } else result = false;
            return result;
        }
            ...
            try
            {
                ...
                gvUsuario.DataSource = (from usuario in gestorSeguridad.ListaUsuario()
                                        select new
                                        {
                                            ...
                                            ID_DOMINIO = usuario.IDENTIFICADOR
                                        }).ToArray();
                gvUsuario.DataBind();
            }
            catch
        ...
        public void CargarUsuariosGridBusqueda(string user, string nombre, string dominio)
        {
            UsuarioDTO _usuarioDto = new UsuarioDTO();
            GestorSeguridad gestorSeguridad = null;
            int x = 1;
            try
            {
                ...
                gvUsuario.DataSource = (from usuario in gestorSeguridad.BuscarUsuario(user, nombre, dominio)
                                        select new
                                        {
                                            NUMERO = x++,
                                            ID_USUARIO = usuario.ID_USUARIO,
        ...
        private void CargarUsuariosEdicion(GridViewCommandEventArgs e)
        {
            try
            {
                ...
                lblUsuarioDB.Text = HttpUtility.HtmlDecode(fila.Cells[10].Text);
                ddlDominio.SelectedValue = HttpUtility.HtmlDecode(fila.Cells[5].Text).ToString();
            }
            catch
            {
                DesplegarError("Error al cargar información de usuario");
            }
        }
        ...
        private void MapearCentrosCosto(GridViewCommandEventArgs e)
        {
            ...
            string url = string.Concat("~/Seguridad/UsuarioSociedadCentro.aspx?", b64);
            Response.Redirect(url);
        }
        ...
        private void AgregarAprobador(GridViewCommandEventArgs e)
        {
            ...
            //string url = string.Concat("~/Seguridad/AprobadorOrdenCosto.aspx?", b64);
            string url = string.Concat("~/Seguridad/AprobadorCentro.aspx?", b64);
            Response.Redirect(url);
        }
        ...
        private void CargarDominio()
        {
           ...
           // GestorCajaChica gestorCC = null;
            ...
            {
                gestorSeguridad = GestorSeguridad();
                listaDDLDto = gestorSeguridad.ListaDominio();

                ddlDominio.DataSource = listaDDLDto;
                ddlDominio.DataTextField = "DESCRIPCION";
                ddlDominio.DataValueField = "IDENTIFICADOR";
                ddlDominio.DataBind();
                ddlDominio.Items.Insert(0, new ListItem("--Seleccione Dominio--", "0"));

            }
            catch
            {
                DesplegarError("Error al recuperar los Dominios");
            }
