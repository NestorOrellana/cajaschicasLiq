...
namespace DipCmiGT.LogicaCajasChicas.Entidad
{
    public class FacturaEncabezado
    {
        ...
        protected string sqlSelectINEL = @" SELECT distinct IdFactura, c.IdTipoDocumento, c.Descripcion, b.NumeroIdentificacion, b.Nombre, Serie, Numero, FechaFactura, 
                                        EsEspecial, RetencionIVA, RetencionISR, IVA, a.ValorTotal, a.Estado, a.UsuarioCreacion, a.FechaCreacion, UsuarioModifico, 
                                        a.FechaModificacion, a.IdProveedor, a.IdCajaChica, CentroCosto, OrdenesCosto, aprobada, Correlativo, NumeroCajaChica, 
                                        d.Estado, a.TipoFactura, a.NumeroRetencionISR, a.NumeroRetencionIVA, a.ValorRetencionISR, 
                                        a.ValorRetencionIVA, d.NumeroCajaChica, Nivel, d.IdSociedadCentro, case when g.Moneda is null then h.Moneda else g.Moneda end Moneda,
                                        ...
                                        (SELECT ISNULL(Z_ERRORES,'Enviado') 
                                         FROM [CC_INELDAT_IPCR].[MFE_IPCR].[dbo].[T_ENCABEZADO_FE_COM] AS Inel 
                                         WHERE Inel.VBELN = a.IdFactura) as EstadoINELDAT
        ...
        protected internal List<FacturaEncabezadoDTO> CargarReader(SqlDataReader sqlDataReader, bool estadoIneldat = false)
        {
            ...
            try
            {
                while (sqlDataReader.Read())
                {
                    ...
                    if (estadoIneldat)
                    {
                        _facturaEncabezadoDto.EstadoINELDAT = sqlDataReader.IsDBNull(50) ? null : sqlDataReader.GetString(50);
                    }
        ...
        {
            SqlCommand _sqlComando = null;
            string sql = sqlSelectINEL + @" WHERE a.IdCajaChica = @IdCajaChica 
                                        and a.Estado = case when @Estado = -1 then a.Estado else @Estado end  
            ...
            return CargarReader(_sqlComando.ExecuteReader(), true);
        }
