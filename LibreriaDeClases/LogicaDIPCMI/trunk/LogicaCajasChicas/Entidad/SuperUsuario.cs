...
//                            AND USUARIO = @usuario ";
            ...
            string sql = @"SELECT TOP 1 * FROM (select top 1 IdUsuarioSociedadCentro from [dbo].[UsuarioSociedadCentro] A 
                                ...
                                SELECT TOP 1 IdUsuarioCentroCosto FROM dbo.UsuarioCentroCosto A 
                                INNER JOIN dbo.SociedadCentro B 
	                                ON A.IdSociedadCentro = B.IdSociedadCentro
                                where A.Usuario = @usuario
	                                and CodigoSociedad = @codigoSociedad 
